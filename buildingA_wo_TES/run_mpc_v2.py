...
# load measurement compensator
import compensator_utility as comp
...
def run_compensator_mpc(measurements, Z, compensator, targets, model):
    # initial measurements (SI units)
    measurements_ini = { 'V_core': [0.]*4,           # 0 ~ 2.8 m3/s 
                            'V_east': [0.]*4,           # 0 ~ 0.9 m3/s
                            'V_north': [0.]*4,          # 0 ~ 0.95 m3/s
                            'V_south': [0.]*4,          # 0 ~ 0.95 m3/s
                            'V_west': [0.]*4,           # 0 ~ 0.70 m3/s
                            'V_total': [0.]*4,          # 0 ~ 4.72 m3/s
                            'T_core': [273.15+24]*4,    # 291.15 ~ 303.15 Kelvin
                            'T_east': [273.15+24]*4,    # 291.15 ~ 303.15 Kelvin
                            'T_north': [273.15+24]*4,   # 291.15 ~ 303.15 Kelvin
                            'T_south': [273.15+24]*4,   # 291.15 ~ 303.15 Kelvin
                            'T_west': [273.15+24]*4,    # 291.15 ~ 303.15 Kelvin
                            'T_outdoor': [273.15+24]*4, # 279.85 ~ 308.26 Kelvin
                            'RH_outdoor': [0.5]*4,      # Unit: 0 ~ 1
                            'P_fan': [0.]*4,            #  0 ~ 7000 W
                            'P_chiller': [0.]*4,        #  0 ~  30000 W
                            'P_chwPump': [0.]*4,        #  0 ~ 1426 W
                            'P_cwPump': [0.]*4,         #  0 ~ 1459 W
                            'P_cooTower': [0.]*4,       #  0 ~ 4300 W
                            'P_totalPower': [0.]*4 }    #  0 ~ 42000 W                            

    # get measurements from the central database (IP to SI units)
    measurements_ini = measurements
    print("\nmeasurements_ini before compensator:",measurements_ini)

    ##----------------------------------------------
    # run compensator and get corrected measurments
    ##----------------------------------------------
    if compensator:
        measurements_ini, targets, model = comp.run_compensator(targets, measurements_ini, Z, model)
    print("\nmeasurements_ini after compensator:",measurements_ini)
    print("target variables:",targets)

    return measurements_comp, targets, model
...
t_start = 212*24*3600  # simulation start time: 207*24*3600 issue day: 218 # three-month training: 7,8,9, start from Day 181 to Day 273 #August 1st is Day 212, September 30th is Day 273
#t_start = 274*24*3600
t_period = 1*24*3600.
...
ZoneMODELPATH = "system_identification/zone_results"
mpc_models['core'] = json.load(open(os.path.join(ZoneMODELPATH, 'TZone_fiveV_Core.json')))
mpc_models['east'] = json.load(open(os.path.join(ZoneMODELPATH, 'TZone_fiveV_East.json')))
mpc_models['west'] = json.load(open(os.path.join(ZoneMODELPATH, 'TZone_fiveV_West.json')))
mpc_models['south'] = json.load(open(os.path.join(ZoneMODELPATH, 'TZone_fiveV_South.json')))
mpc_models['north'] = json.load(open(os.path.join(ZoneMODELPATH, 'TZone_fiveV_North.json')))
...
##========load data for compensator=============#
if compensator:
    data_fname = 'compensator\\dataset_mpc_compensator.csv'
    var_fname = 'compensator\\mpc_inputs.txt'
    Z = comp.load_train_data(data_fname, var_fname)
    #targets = ['mod.flo.temAirPer5.T']
    targets = ['mod.TSup.T']
    model = None
...
while ts < t_end:
    ...
    if not warmup:  # activate mpc after warmup
        ...
        # uMPC = [res['x'][0], res['x'][1], res['x'][2]+273.15, res['x'][3]+273.15, res['x']
        #         [4]+273.15, 2.5, 0., 0.95, 0.95, res['x'][9]] # Veast setpoint = 0
        # uMPC = [res['x'][0], res['x'][1], res['x'][2]+273.15, res['x'][3]+273.15, res['x']
        #         [4]+273.15, res['x'][5], res['x'][6], res['x'][7], res['x'][8], 0.] # One zone was attacked
    ...
    if dos_attack_core_VAV:
        ...
        print("\nmeasurement after DoS attack:", measurement)
    
    if compensator:
        # print original measurement targets
        print("\nmeasurement targets:", measurement[targets])
        
        # emulate False data intrusion attack on VAV Tsup measurement [12, 18] C
        measurement.at[measurement.index[0],'mod.TSup.T'] = 273.15 + 30 # df.at[] is faster than df.loc[]
        print("\nmeasurement after False data intrusion attack:", measurement[targets])
        
        # run compensator and get corrected measurments
        measurements_comp, targets, model = comp.run_compensator(targets, measurement, Z, model)
        print("\nmeasurement corrected by compensator:", measurements_comp[targets])
    ...
    outputs_dict = {}
    output_names = measurement_names + input_control
    for output_name in output_names:
        outputs_dict[output_name] = res[output_name]
    if compensator:
        for target in targets:
            outputs_dict['compensated-'+target] = measurements_comp[target]
...
if dos_attack_core_VAV:
    ...
    results.to_csv('mpc_results/results_DoS_attack_core_VAV_opt_PH'+str(PH)+'.csv')
elif compensator:
    final['compensator'] = True
    with open('mpc_results/mpc_results_compensator.json', 'w') as outfile:
        json.dump(final, outfile)
    results.to_csv('mpc_results/results_compensator_opt_PH'+str(PH)+'.csv')
else: 
    with open('mpc_results/mpc_results.json', 'w') as outfile:
        json.dump(final, outfile)
    results.to_csv('mpc_results/results_opt_PH'+str(PH)+'.csv')
print("\n MPC simulation is completed!")
